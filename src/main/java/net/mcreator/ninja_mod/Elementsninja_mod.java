...
public class Elementsninja_mod implements IFuelHandler, IWorldGenerator {
	...
	public void preInit(FMLPreInitializationEvent event) {
		try {
			for (ASMDataTable.ASMData asmData : event.getAsmData().getAll(ModElement.Tag.class.getName())) {
				...
				if (clazz.getSuperclass() == Elementsninja_mod.ModElement.class)
					elements.add((Elementsninja_mod.ModElement) clazz.getConstructor(this.getClass()).newInstance(this));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		Collections.sort(elements);
		elements.forEach(Elementsninja_mod.ModElement::initElements);
		this.addNetworkMessage(ninja_modVariables.WorldSavedDataSyncMessageHandler.class, ninja_modVariables.WorldSavedDataSyncMessage.class,
	...
	public void onPlayerLoggedIn(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent event) {
		if (!event.player.world.isRemote) {
			WorldSavedData mapdata = ninja_modVariables.MapVariables.get(event.player.world);
			WorldSavedData worlddata = ninja_modVariables.WorldVariables.get(event.player.world);
			if (mapdata != null)
				ninja_mod.PACKET_HANDLER.sendTo(new ninja_modVariables.WorldSavedDataSyncMessage(0, mapdata), (EntityPlayerMP) event.player);
			if (worlddata != null)
				ninja_mod.PACKET_HANDLER.sendTo(new ninja_modVariables.WorldSavedDataSyncMessage(1, worlddata), (EntityPlayerMP) event.player);
		}
	}
	...
	public <T extends IMessage, V extends IMessage> void addNetworkMessage(Class<? extends IMessageHandler<T, V>> handler, Class<T> messageClass,
			Side... sides) {
		for (Side side : sides)
			ninja_mod.PACKET_HANDLER.registerMessage(handler, messageClass, messageID, side);
		messageID++;
	}
	...
	public static class ModElement implements Comparable<ModElement> {
		...
		protected final Elementsninja_mod elements;
		protected final int sortid;

		public ModElement(Elementsninja_mod elements, int sortid) {
